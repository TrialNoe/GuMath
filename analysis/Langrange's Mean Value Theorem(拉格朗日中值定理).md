...
## 2 运用Lagrange's Mean Value Theorem
...
- [[证明函数单调性的性质]]：我们已知**导函数的正负的信息**，要**推导原函数两个不同位置的值的大小的信息**，通过拉格朗日中值定理并且做差（刚好把常数c的影响消除了）来比较大小
- [[恒等式的常值函数证法#原理]]：**导数为0的信息推导原函数为常函数**也完美契合中心思想


### 2.2 区间拆分法

- [[多个独立点的Lagrange's Mean Value Theorem 问题]]：通过对区间拆分，做成两个区间就可以在一个函数中分别用两个不同的点使用拉格朗日
